...
import { getAboutUsList, deleteAboutUs } from './lib/api';
...
export default function AboutUsManagement() {
  ...
  const loadAboutUs = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'فشل في تحميل عن الأوروبية');
      toast.error('فشل في تحميل عن الأوروبية');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      await loadAboutUs();
      toast.success('تم حذف عن الأوروبية بنجاح');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'فشل في حذف عن الأوروبية');
      toast.error('فشل في حذف عن الأوروبية');
    }
  };
  ...
  return (
    <div className="container mx-auto p-6" dir="rtl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">عن الأوروبية</h1>
        <button
          onClick={() => router.push('/dashboard/about-us/add')}
              ...
              aboutUsItems.map((item) => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 flex gap-2">
                    <button
                      onClick={() => router.push(`/dashboard/about-us/edit/${item.id}`)}
