...
import { createAboutUs } from '../lib/api';
...
export default function AddAboutUs() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('تم إنشاء عن الأوروبية بنجاح');
      router.push('/dashboard/about-us');
      router.refresh();
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : 'فشل في إنشاء عن الأوروبية';
      setError(errorMsg);
  ...
  return (
    <div className="container mx-auto" dir="rtl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">إضافة عن الأوروبية</h1>
        <button
          onClick={() => router.back()}
