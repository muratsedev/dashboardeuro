...
import { getAboutUsById, updateAboutUs } from '../../lib/api';
import RichTextEditor from '../../../../../components/RichTextEditor';
import { AboutUs } from '../../types/AboutUs';

// Force dynamic rendering for dashboard authentication
...
export default function EditAboutUs() {
  ...
  const [formData, setFormData] = useState<AboutUs | null>(null);
  ...
  useEffect(() => {
    const loadAboutUs = async () => {
      try {
        const data = await getAboutUsById(parseInt(id));
        setFormData(data);
      } catch (err) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    setFormData((prev: AboutUs | null) => prev ? { ...prev, [name]: value } : null);
  };

  const handleContentChange = (content: string): void => {
    setFormData((prev: AboutUs | null) => prev ? { ...prev, content } : null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateAboutUs(formData.id, {
        id: formData.id,
        title: formData.title,
        content: formData.content
      });
      toast.success('تم تحديث عن الأوروبية بنجاح');
      router.push('/dashboard/about-us');
      router.refresh();
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : 'فشل في تحديث عن الأوروبية';
      setError(errorMsg);
      toast.error(errorMsg);
  ...
  return (
    <div className="container mx-auto" dir="rtl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">تعديل عن الأوروبية</h1>
        <button
          onClick={() => router.back()}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="أدخل عنوان عن الأوروبية"
            />
          </div>
          ...
          <div>
            ...
            <RichTextEditor
              ...
              placeholder="أدخل محتوى عن الأوروبية"
            />
          </div>
