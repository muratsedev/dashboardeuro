...
import { getUpperArticle, updateUpperArticle } from "../../lib/api";
import { UpdateUpperArticleDto } from "../../types/UpperArticle";
import { toast } from "react-hot-toast";


export default function EditUpperArticle({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter();
  const { id } = use(params);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isLoading, setIsLoading] = useState(true);
  const [formData, setFormData] = useState<UpdateUpperArticleDto>({
    upperArticleName: "",
  });
  ...
  useEffect(() => {
    const fetchUpperArticle = async () => {
      try {
        setIsLoading(true);
        const upperArticle = await getUpperArticle(Number(id));
        setFormData({
          upperArticleName: upperArticle.upperArticleName,
        });
      } catch (error) {
        console.error("Error fetching upper article:", error);
        toast.error("فشل في تحميل بيانات المقال العلوي", {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateUpperArticle(Number(id), formData);

      toast.success("تم تحديث المقال العلوي بنجاح", {
        position: "bottom-center",
        duration: 3000,
        style: {
          background: "#10B981",
      ...
      console.error("Error updating upper article:", error);
      toast.error("فشل في تحديث المقال العلوي", {
        position: "bottom-center",
        duration: 3000,
        style: {
          background: "#EF4444",
