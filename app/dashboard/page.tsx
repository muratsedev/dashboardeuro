  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const [articlesRes, categoriesRes, breakingNewsRes] = await Promise.all([
          fetch(getApiEndpoint('/api/Articles')),
          fetch(getApiEndpoint('/api/Categories')),
          fetch(getApiEndpoint('/api/BreakingNews'))
        ]);

        if (articlesRes.ok && categoriesRes.ok && breakingNewsRes.ok) {
          const articles = await articlesRes.json();
          const categories = await categoriesRes.json();
          const breakingNews = await breakingNewsRes.json();
          ...
          const recentArticles = articles.filter((article: any) => 
            new Date(article.publishDate || article.createdAt) > oneMonthAgo
          ).length;
          ...
          setStats({
            ...
            totalBreakingNews: breakingNews.length || 0,
            publishedArticles,
            recentArticles,
