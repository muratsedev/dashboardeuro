...
import { getTermsOfUseList, deleteTermsOfUse } from './lib/api';

export default function TermsOfUseManagement() {
  const router = useRouter();
  const [termsOfUseList, setTermsOfUseList] = useState<TermsOfUse[]>([]);
  ...
  const loadTermsOfUse = async () => {
    try {
      ...
      setTermsOfUseList(data);
      setError('');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'فشل في تحميل شروط الاستخدام');
      toast.error('فشل في تحميل شروط الاستخدام');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await deleteTermsOfUse(id);
      await loadTermsOfUse();
      toast.success('تم حذف شروط الاستخدام بنجاح');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'فشل في حذف شروط الاستخدام');
      toast.error('فشل في حذف شروط الاستخدام');
    }
  };
  ...
  return (
    <div className="container mx-auto p-6" dir="rtl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">شروط الاستخدام</h1>
        <button
          onClick={() => router.push('/dashboard/terms-of-use/add')}
              ...
              termsOfUseList.map((item) => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 flex gap-2">
                    <button
                      onClick={() => router.push(`/dashboard/terms-of-use/edit/${item.id}`)}
