...
import { createTermsOfUse } from '../lib/api';
...
export default function AddTermsOfUse() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createTermsOfUse(formData);
      toast.success('تم إنشاء شروط الاستخدام بنجاح');
      router.push('/dashboard/terms-of-use');
      router.refresh();
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : 'فشل في إنشاء شروط الاستخدام';
      setError(errorMsg);
  ...
  return (
    <div className="container mx-auto" dir="rtl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">إضافة شروط استخدام جديدة</h1>
        <button
          onClick={() => router.back()}
