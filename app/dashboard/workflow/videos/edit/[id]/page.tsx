...
import { getVideo, updateVideo } from '@/app/dashboard/videos/lib/api';
    ...
    getVideo(id)
      .then((v) => {
        setForm({
          ...
          videoSummary: v.videoSummary ?? '',
          videoLink: v.videoLink,
          isPublished: v.isPublished,
        });
      })
      .catch(() => toast.error('فشل تحميل بيانات الفيديو'))
      .finally(() => setLoading(false));
  }, [id]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.videoTitle.trim() || !form.videoLink.trim()) {
    ...
    try {
      ...
      router.push('/dashboard/workflow/videos');
    } catch {
  ...
  return (
    <div dir="rtl" className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">تعديل الفيديو</h1>
        <button
          onClick={() => router.back()}
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 space-y-5">
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) => setForm({ ...form, videoSummary: e.target.value })}
            rows={3}
