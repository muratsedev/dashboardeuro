...
import { getPodcast, updatePodcast } from '@/app/dashboard/podcasts/lib/api';
    ...
    getPodcast(id)
      .then((p) => {
        setForm({
          ...
          podcastSummary: p.podcastSummary ?? '',
          podcastLink: p.podcastLink,
          isPublished: p.isPublished,
        });
      })
      .catch(() => toast.error('فشل تحميل بيانات البودكاست'))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updatePodcast(id, {
        ...
        podcastSummary: form.podcastSummary || undefined,
        podcastLink: form.podcastLink,
        isPublished: form.isPublished,
      });
      toast.success('تم تحديث البودكاست بنجاح');
      router.push('/dashboard/workflow/podcasts');
    } catch {
      toast.error('فشل تحديث البودكاست');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div dir="rtl" className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">تعديل البودكاست</h1>
        <button
          onClick={() => router.back()}
