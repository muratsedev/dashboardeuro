...
export default function CreateSocialMediaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedFile) {
      const errorMsg = 'يجب تحميل أيقونة';
      setError(errorMsg);
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: 'Failed to create social media' }));
      ...
      const errorMessage = err instanceof Error ? err.message : 'فشل في إضافة وسيلة التواصل';
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">إضافة وسيلة تواصل اجتماعي</h1>
      </div>
              ...
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  ...
                  <p className="text-sm text-gray-500">
                    {selectedFile ? selectedFile.name : 'اضغط لتحميل أيقونة'}
                  </p>
                </div>
                <input
                  type="file"
