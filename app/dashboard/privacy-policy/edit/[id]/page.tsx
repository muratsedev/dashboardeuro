...
import { getPrivacyPolicyById, updatePrivacyPolicy } from '../../lib/api';
...
export default function EditPrivacyPolicy() {
  ...
  useEffect(() => {
    const loadPrivacyPolicy = async () => {
      try {
        const data = await getPrivacyPolicyById(parseInt(id));
        setFormData(data);
      } catch (err) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    setFormData((prev: PrivacyPolicy | null) => prev ? { ...prev, [name]: value } : null);
  };

  const handleContentChange = (content: string): void => {
    setFormData((prev: PrivacyPolicy | null) => prev ? { ...prev, content } : null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updatePrivacyPolicy(formData.id, {
      ...
      const errorMsg = err instanceof Error ? err.message : 'فشل في تحديث سياسة الخصوصية';
      setError(errorMsg);
      toast.error(errorMsg);
  ...
  return (
    <div className="container mx-auto" dir="rtl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">تعديل سياسة الخصوصية</h1>
        <button
          onClick={() => router.back()}
